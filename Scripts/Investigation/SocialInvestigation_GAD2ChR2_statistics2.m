...
XX = 1; % including only the first XXs
for i = GAD2_Exc
    %
    if ~isempty(S(i).Social)==1
        ...
        for j=1:length(NovObj)
            ...
            if ~isempty(NovObj{j}.Follow_start) && NovObj{j}.Follow_end(1)<XX
           ...
           %
            ...
            if ~isempty(NovObj{j}.Accept_start) && NovObj{j}.Accept_end(1)<XX
            ...
            if ~isempty(NovObj{j}.Anogenital_start) && NovObj{j}.Anogenital_end(1)<XX
            ...
            if ~isempty(NovObj{j}.Facial_start) && NovObj{j}.Facial_end(1)<XX
            ...
            if ~isempty(NovObj{j}.Body_start) && NovObj{j}.Body_end(1)<XX
